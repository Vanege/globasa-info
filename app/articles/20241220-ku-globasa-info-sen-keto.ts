import { Article } from "../types";

export const article: Article = {
  title: "Ku globasa.info sen keto",
  author: "",
  imageUrl: "",
  imageDescription: "",
  isPublished: true,
  content: `
### Presenta

globasa.info sen makale-netodom
* Makale hare **jandan jumle** ji **pimpan lexi**. Hinloka sen bon cel xoruyen!
* Na perya lexi onexa basatayti!
* Moy texto in moy makale sen **demo-domeni-li** (Public Domain CC0). Moyto haki na riyongu Globasa-li inhare.
* Makale abil na **tema moyto**. globasa.info-tim cuyo eskri tem plu neo xey in Globasa cel na diskonegi oto.
* Cele sen xunjan de Globasa ji de Globasa-komuntim.

Kam yu vole na sahay globasa.info-tim? Am mesaje imi in [resmi Discord de Globasa](https://discord.gg/JCaqAvapGR).

### Izindoku

* Moy texto in moy makale sen demo-domeni-li [CC0](https://creativecommons.org/public-domain/cc0/).
* Moyun imaje hare suli izindoku. Am oko to in makale, fe comen de imaje.
* Rukesahni de globasa.info sen [Free SVG Backgrounds and Patterns de SVGBackgrounds.com](https://www.svgbackgrounds.com/set/free-svg-backgrounds-and-patterns/).
`
};